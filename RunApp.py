...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        vo= str(os.path.abspath(os.getcwd()))+r'\ico.ico'
        self.iconbitmap(vo)
        ...
        self.label_radio_group = customtkinter.CTkLabel(master=self.frame_right,
                                                        text="Settings",text_font=("Roboto Medium", -16))
        ...
        self.check_box_1.configure(state=tkinter.DISABLED, text="French")
...
############ FUNCTION TO CLEAN THE DATA ###############
    ...
    def read_file(self):
        ...
        path= str(os.path.abspath(os.getcwd()))+r"\tmp" +"\\"+ c + ".xlsx"
        path1= str(os.path.abspath(os.getcwd()))+r"\tmp"+"\\"+"df.csv"
        ...
        clusterpath= str(os.path.abspath(os.getcwd()))+r"\tmp"+"\\"+"datareg.csv"
...
#########################################################################################################
    ...
    def cluster_data(self):
        ...
        global c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17,c18,c19,c20,c21,c22,c23,c24,c25,c26,c27,c28,c29
        ...
        for k in range(30):
            y='c'+str(k)
            p=y +'.xlsx'
            y=dataclean[dataclean['Cluster']==k]
            pathf= str(os.path.abspath(os.getcwd()))+r"\save_as"+"\\"+p
            y.to_excel(pathf, index = True ,header=True) 
    
            
        #save sous clusters
        clus=[c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17,c18,c19,c20,c21,c22,c23,c24,c25,c26,c27,c28,c29]
       

        for i in clus: 
            
            if i.shape[0]>90:
                clss=joblib.load('modele8.sav')
                clusterss=clss.fit_predict(i)
                i.insert(0, "Clusterss", clusterss, True)
                for j in range(8):
                    x=self.get_df_name(i)+'_'+str(j)
                    x=i[i['Clusterss']==j]
                    p=self.get_df_name(i)+'_'+str(j)+'.xlsx'
                    pathf= str(os.path.abspath(os.getcwd()))+r"\save_as"+"\\"+p
                    x.to_excel(pathf, index = True ,header=True)
                
            elif (i.shape[0]>20 and i.shape[0]<50) : 
                clss=joblib.load('modele2.sav')
                clusterss=clss.fit_predict(i)
                i.insert(0, "Clusterss", clusterss, True)
                for j in range(2):
                    x=self.get_df_name(i)+'_'+str(j)
        ...
        Message='File 1 saved as : '+'\n' +  str(os.path.abspath(os.getcwd()))+r"\save_as"
        self.label_info_1 = customtkinter.CTkLabel(master=self.frame_info,
                                                   text= Message,
                                                   height=100,
                                                   fg_color=("white", "gray38"),  # <- custom tuple-color
...
class Example(Frame):
        ...
        def initUI(self):
            ...
            editMenu.add_command(label="Analyse", command=self.analyse)
        ...
        def analyse(self):
            ...
            new_window.transient(self.master)
            new_window.title("Analyse")
            
            
            #frame =tkinterhtml.HtmlFrame(new_window,horizontal_scrollbar="auto") #create HTML browser
            my_label = HTMLLabel(new_window, html="""
                    <html>
                    <head></head>
                    <body>
                    <h2>Contoso Retail Worldwide Report</h2>
                    
                    <a href="https://app.powerbi.com/reportEmbed?reportId=e7a66a17-9821-4656-966b-b6e21a4b8ed6&autoAuth=true&ctid=889cdbee-d881-42dc-bd06-ad3237c34a53&config=eyJjbHVzdGVyVXJsIjoiaHR0cHM6Ly93YWJpLXNvdXRoLWFmcmljYS1ub3J0aC1hLXByaW1hcnktcmVkaXJlY3QuYW5hbHlzaXMud2luZG93cy5uZXQvIn0%3D" >DASHBOARD </a>
                   
                    </div>
                    <hr class="my-4">
                    <div class="pt-5">
                    <h6 class="mb-0"> <a  href="/"  class="mb-0"> <i class="fas fa-long-arrow-alt-left me-2"></i>   Back </a> </h6>
                    </div>
                    </body>
                    </html>
                """)
